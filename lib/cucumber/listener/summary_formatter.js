function SummaryFormatter(options) {
  ...
  self.isSameType = function isSameType(a, b) {
  ...
  self.storeFailedStepResult = function storeFailedStepResult(stepResult) {
    ...
    var message;
    if (error.showDiff && error.expected !== undefined && self.isSameType(error.actual, error.expected)) {
      var actual = util.format(error.actual);
      var expected = util.format(error.expected);
      var unifiedDiff = diff.createPatch('string', actual, expected)
        .split('\n')
        .splice(4)
        .map(function(line) {
          if (line[0] === '+') {
            return colors.diffAdded(line);
          }
          if (line[0] === '-') {
            return colors.diffRemoved(line);
          }
          if (line.match(/\@\@/)) {
            return null;
          }
      ...
      var messageIndex = error.stack.indexOf(error.message)
      if (messageIndex !== -1) {
        stack = stack.slice(messageIndex + error.message.length);
      }
      message = colors.diffAdded('+ expected') + ' ' + colors.diffRemoved('- actual') +
        '\n\n' + colors.diffEqual(unifiedDiff) +
        '\n' + stack;
    } else {
